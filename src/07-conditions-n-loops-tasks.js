...
function isInsideCircle(circle, point) {
  const gap = (point.x - circle.center.x) * (point.x - circle.center.x)
  + (point.y - circle.center.y) * (point.y - circle.center.y);
  if (gap < circle.radius ** 2) {
    return true;
  }
  return false;
}
...
function reverseString(str) {
  const seekingArr = str.split('');
  return seekingArr.reverse().join('');
}
...
function reverseInteger(num) {
  const seek = num.toString();
  const seekingArr = [];
  for (let i = 0; i < seek.length; i += 1) {
    seekingArr.push(seek[i]);
  }
  return +seekingArr.reverse().join('');
}
